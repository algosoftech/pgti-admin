...
export const list = async (options={}) =>{
    try {
        const params = {
            url : `${API_K_WINN_URL}./admin/users/list`,
            postData : options
        }
...
export const addEditUsers = async (options={}) =>{
    try {
        const params = {
            url : `${API_K_WINN_URL}./admin/users/addeditdata`,
            postData : options
        }
        const res = await postRequest(params);
        if(res.status === true || res.status === 200){
...
export const usersChangeStatus = async (options={}) =>{
    try {
        const params = {
            url : `${API_K_WINN_URL}./admin/users/change-status`,
            postData : options
        }
        const res = await postRequest(params);
        if(res.status === true || res.status === 200){
...
export const updateAmount = async (options={}) =>{
    try {
        const params = {
            url : `${API_K_WINN_URL}./admin/users/update-amount`,
            postData : options
        }
        const res = await postRequest(params);
        if(res.status === true || res.status === 200){
