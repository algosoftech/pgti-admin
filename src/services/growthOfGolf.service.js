...
export const listGrowthOfGolf = async (options = {}) => {
  try {
    const res = await postRequest({ url: `${BASE}/admin/cms/growth-of-golf/list`, postData: { ...options } });
    if (res?.status === 200 && res?.data?.status) {
      return { status: true, result: res?.data?.response?.result || null };
    }
    return { status: false, message: res?.data?.message || 'Failed to fetch Growth of Golf data' };
  } catch (error) {
    return { status: false, message: error.message || 'Network error' };
  }
};

export const addEditGrowthOfGolf = async (options = {}) => {
  try {
    const { editId, ...rest } = options;
    const postData = { ...rest, ...(editId && { edit_id: editId }) };
    const res = await postRequest({ url: `${BASE}/admin/cms/growth-of-golf/add-edit`, postData });
    if (res?.status === 200 && res?.data?.status) {
...
export const changeGrowthOfGolfStatus = async ({ id, status }) => {
  try {
    const res = await postRequest({ url: `${BASE}/admin/cms/growth-of-golf/change-status`, postData: { id, status } });
    if (res?.status === 200 && res?.data?.status) {
      return { status: true };
    }
