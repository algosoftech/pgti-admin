...
export const getGalleryListingBanner = async (tourType = "M") => {
  try {
    ...
    return { status: false, message: extractApiError(res, 'Failed to fetch gallery listing banner') };
  } catch (error) {
    return { status: false, message: extractApiError(error, 'Request failed') };
  }
};

export const addEditGalleryListingBanner = async (options = {}) => {
  try {
    const res = await postRequest({ url: `${BASE}/admin/cms/gallery/banner/add-edit`, postData: options });
    if (res?.status === 200 && res?.data?.status) {
      return { status: true, result: res?.data?.response?.result };
    }
    return { status: false, message: extractApiError(res, 'Failed to save gallery listing banner') };
  } catch (error) {
    return { status: false, message: extractApiError(error, 'Request failed') };
  }
};
