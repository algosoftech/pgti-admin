...
export const listTermsConditions = async (options = {}) => {
  try {
    const res = await postRequest({ url: `${BASE}/admin/cms/terms-conditions/list`, postData: { ...options } });
    if (res?.status === 200 && res?.data?.status) {
      return { status: true, result: res?.data?.response?.result || null };
    }
        return { status: false, message: res?.data?.message || 'Failed to fetch Terms & Conditions data' };
    } catch (error) {
        return { status: false, message: error.message || 'Network error' };
    }
};

export const addEditTermsConditions = async (options = {}) => {
    try {
        const { editId, ...rest } = options;
        const postData = { ...rest, ...(editId && { edit_id: editId }) };
        const res = await postRequest({ url: `${BASE}/admin/cms/terms-conditions/add-edit`, postData });
        if (res?.status === 200 && res?.data?.status) {
