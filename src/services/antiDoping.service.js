...
export const listAntiDoping = async (options = {}) => {
    try {
        const res = await postRequest({ url: `${BASE}/admin/cms/anti-doping/list`, postData: { ...options } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result || null };
        }
        return { status: false, message: res?.data?.message || 'Failed to fetch Anti-Doping data' };
    } catch (error) {
        return { status: false, message: error.message || 'Network error' };
    }
};

export const addEditAntiDoping = async (options = {}) => {
    try {
        const { editId, ...rest } = options;
        const postData = { ...rest, ...(editId && { edit_id: editId }) };
        const res = await postRequest({ url: `${BASE}/admin/cms/anti-doping/add-edit`, postData });
        if (res?.status === 200 && res?.data?.status) {
...
export const changeAntiDopingStatus = async ({ id, status }) => {
    try {
        const res = await postRequest({ url: `${BASE}/admin/cms/anti-doping/change-status`, postData: { id, status } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true };
        }
