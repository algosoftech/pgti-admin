...
export const list = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/product-variants/list`, postData: { page, limit, ...rest } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result, count: res?.data?.response?.count };
        }
        return { status: false, message: extractApiError(res, 'Failed to fetch product variants') };
    } catch (error) {
...
export const addEditProductVariant = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/product-variants/add-edit`, postData });
        if (res?.status === 200 && res?.data?.status) {
...
export const productVariantChangeStatus = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/product-variants/change-status`, postData: { id: id || editId, status } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result };
        }
