...
export const list = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/contact-shops/list`, postData: { page, limit, ...rest } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result, count: res?.data?.response?.count };
        }
        return { status: false, message: extractApiError(res, 'Failed to fetch contact shops') };
    } catch (error) {
...
export const addEditContactShop = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/contact-shops/add-edit`, postData });
        if (res?.status === 200 && res?.data?.status) {
...
export const contactShopChangeStatus = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/contact-shops/change-status`, postData: { id: id || editId, status } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result };
        }
...
export const deleteContactShop = async (options = {}) => {
    try {
        const res = await postRequest({ url: `${BASE}/admin/contact-shops/delete`, postData: options });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result };
        }
