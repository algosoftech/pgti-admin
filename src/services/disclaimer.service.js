...
export const listDisclaimer = async (options = {}) => {
    try {
        const res = await postRequest({ url: `${BASE}/admin/cms/disclaimer/list`, postData: { ...options } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result || null };
        }
        return { status: false, message: res?.data?.message || 'Failed to fetch Disclaimer data' };
    } catch (error) {
        return { status: false, message: error.message || 'Network error' };
    }
};

export const addEditDisclaimer = async (options = {}) => {
    try {
        const { editId, ...rest } = options;
        const postData = { ...rest, ...(editId && { edit_id: editId }) };
        const res = await postRequest({ url: `${BASE}/admin/cms/disclaimer/add-edit`, postData });
        if (res?.status === 200 && res?.data?.status) {
