...
export const list = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/products/list`, postData: { page, limit, ...rest } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result, count: res?.data?.response?.count };
        }
        return { status: false, message: extractApiError(res, 'Failed to fetch products') };
    } catch (error) {
...
export const addEditProduct = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/products/add-edit`, postData });
        if (res?.status === 200 && res?.data?.status) {
...
export const productChangeStatus = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/products/change-status`, postData: { id: id || editId, status } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result };
        }
