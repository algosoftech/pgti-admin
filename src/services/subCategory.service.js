...
export const list = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/sub-categories/list`, postData: { page, limit, ...rest } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result, count: res?.data?.response?.count };
        }
        return { status: false, message: extractApiError(res, 'Failed to fetch sub-categories') };
    } catch (error) {
...
export const addEditSubCategory = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/sub-categories/add-edit`, postData });
        if (res?.status === 200 && res?.data?.status) {
...
export const subCategoryChangeStatus = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/sub-categories/change-status`, postData: { id: id || editId, status } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result };
        }
