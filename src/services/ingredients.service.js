...
export const list = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/ingredients/list`, postData: { page, limit, ...rest } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result, count: res?.data?.response?.count };
        }
        return { status: false, message: extractApiError(res, 'Failed to fetch ingredients') };
    } catch (error) {
...
export const addEditIngredients = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/ingredients/add-edit`, postData });
        if (res?.status === 200 && res?.data?.status) {
...
export const ingredientChangeStatus = async (options = {}) => {
    try {
        ...
        const res = await postRequest({ url: `${BASE}/admin/ingredients/change-status`, postData: { id: id || editId, status } });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result };
        }
...
export const deleteIngredient = async (options = {}) => {
    try {
        const res = await postRequest({ url: `${BASE}/admin/ingredients/delete`, postData: options });
        if (res?.status === 200 && res?.data?.status) {
            return { status: true, result: res?.data?.response?.result };
        }
