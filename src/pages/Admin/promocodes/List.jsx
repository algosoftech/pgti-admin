...
export default function PromocodeList() {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          const type = getValue();
          return (type === 2 || type === "2") ? 'Percentage (%)' : type === 1 ? 'Fixed Amount' : type;
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-muted">{moment(row.original.valid_from).format("MMM DD, YYYY")}</div>
        ),
        size: 190,
        enableSorting: true,
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-muted">{moment(row.original.valid_to).format("MMM DD, YYYY")}</div>
        ),
        size: 160,
        enableSorting: true,
      },
  ...
  const getList = () => {
    const options = {
      type: "",
      condition: {
        ...(serverColumnFilters.code ? { code: serverColumnFilters.code } : null),
        ...(serverColumnFilters.discount_type ? { discount_type: serverColumnFilters.discount_type } : null),
