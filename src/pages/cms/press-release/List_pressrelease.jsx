...
export default function PressReleaseList() {
  ...
  const PERMISSION = usePermissions("press_release");
  ...
  const getList = useCallback(async (page = currentPage, pageLimit = limit) => {
    setIsLoading(true);
    const response = await listPressRelease({
  ...
  const handleEdit = (item = {}) => {
    ...
    navigate("/admin/cms/press-release/addeditdata", { state });
  };

  const loadListingBanner = useCallback(async () => {
    const response = await getPressReleaseListingBanner(activeTab === "F" ? "F" : "M");
    if (response?.status) {
  ...
  const openListingBannerEditor = () => {
    navigate("/admin/cms/press-release/listing-banner", { state: listingBanner || { tour_type: activeTab === "F" ? "F" : "M" } });
  };
  ...
  const confirmBannerEdit = () => {
    Modal.confirm({
      title: "Edit press release listing banner?",
      icon: <ExclamationCircleOutlined />,
      content: "This will open the press release listing banner editor. Do you want to continue?",
      okText: "Yes, Edit",
      cancelText: "Cancel",
      onOk: () => {
  ...
  const handleChangeStatus = async (id, status) => {
    try {
      const result = await changePressReleaseStatus({ id, status });
      if (!result?.status) throw new Error(result?.message || "Failed to update status.");
      toast("Success", "Press release status updated successfully.", true);
      getList();
    } catch (error) {
  ...
  const handleDelete = (item) => {
    Modal.confirm({
      title: "Delete this press release item?",
      icon: <ExclamationCircleOutlined />,
      content: item?.title || "This press release item will be removed permanently.",
      okText: "Yes, Delete",
      okType: "danger",
      cancelText: "Cancel",
      onOk: async () => {
        try {
          const result = await deletePressRelease({ id: item.id });
          if (!result?.status) throw new Error(result?.message || "Failed to delete press release item.");
          toast("Success", "Press release item deleted successfully.", true);
          getList();
        } catch (error) {
          toast("Oops!", error?.message || "Failed to delete press release item.");
        }
      },
    });
  };
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorKey: "title",
        header: "Press Release Card",
        cell: ({ row }) => (
          <div>
            <div className="font-weight-600">{row.original.title || "Untitled Press Release"}</div>
      ...
      {
        accessorKey: "press_release_month",
        header: "Month",
  ...
  useEffect(() => {
    loadListingBanner();
    document.title = "PGTI || Admin || Press Release";
    window.scrollTo({ top: 0, behavior: "smooth" });
  }, [loadListingBanner]);

  return (
    <div className="admin-page-container" ref={targetRef}>
      <Top_navbar title="Press Release" />
      ...
      <div className="content-card">
          ...
          <div className="tabs-actions">
            ...
            {canEdit && (
              <button className="action-button primary" onClick={() => handleEdit()}>
                <FontAwesomeIcon icon={faPlus} /> Add Press Release
              </button>
            )}
          </div>
        </div>
        ...
        <div className="content-card-body">
          ...
          <EnhancedTable
            ...
            emptyStateMessage="No press release items found"
            activeTab={activeTab}
            targetRef={targetRef}
            exportFileName="press-release-items"
          />

          <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 8, color: "#64748b", fontSize: 13 }}>
            <FontAwesomeIcon icon={faNewspaper} />
            <span>
              These press release cards power the front Press Release page and its event filters.
            </span>
          </div>
        </div>
      </div>
      ...
      <ListingBannerPreviewModal
        ...
        title="Press Release Listing Banner Preview"
        description="Review the current banner that will be sent in the front Press Release listing API."
