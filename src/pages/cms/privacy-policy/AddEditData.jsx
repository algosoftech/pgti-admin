...
import {
  AlignLeftOutlined, AppstoreOutlined, ArrowLeftOutlined,
  CheckCircleOutlined, CloseOutlined, DeleteOutlined, DownOutlined,
  EditOutlined, ExclamationCircleOutlined, FontSizeOutlined,
  InfoCircleOutlined, PlusOutlined, SaveOutlined, UpOutlined,
} from "@ant-design/icons";
import { Link, useLocation } from "react-router-dom";
import ReactQuill from "react-quill";
import "react-quill/dist/quill.snow.css";
import { addEditPrivacyPolicy, listPrivacyPolicy } from "services/privacyPolicy.service";
...
export default function PrivacyPolicyAddEditData() {
  ...
  const hydrateForm = (record = {}) => {
    ...
    setSections(c.sections); setSavedSections(c.sections);
  ...
  useEffect(() => {
    document.title = `PGTI || ${id ? "Edit" : "Setup"} Privacy Policy`;
    let active = true;
    const load = async () => {
      if (state && Object.keys(state).length > 0 && state.id) hydrateForm(state);
      try {
        setIsFetching(true);
        const res = await listPrivacyPolicy({ tour_type: tourType });
  ...
  const cancelEditingSection = (sectionKey) => {
    if (sectionKey === "header") { setHeader(savedHeader); setStatus(savedStatus); setTourType(savedTourType); }
  ...
  const copyFromMainTour = async () => {
    try {
      setIsFetching(true);
      const res = await listPrivacyPolicy({ tour_type: "M" });
      if (!res?.status || !res?.result?.id) {
        notification.warning({
          message: "Main Tour data not found",
          description: "Please save the Main Tour Privacy Policy page first.",
          placement: "topRight",
          duration: 3,
        });
        return;
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        try {
          ...
          const res = await addEditPrivacyPolicy({
      ...
      <div className="page-header">
        <div className="d-flex justify-content-between align-items-center">
          <div>
            ...
            <p className="page-subtitle">Manage the Privacy Policy page content section by section.</p>
      ...
      <div className="page-body">
                ...
                <div className="row">
                  ...
                  <div className="col-md-6 col-12 mb-3">
                    <div className="form-group">
                      ...
                      <input className="form-input" value={header.title} onChange={(e) => setHeader((prev) => ({ ...prev, title: e.target.value }))} placeholder="e.g. Privacy Policy" />
                    </div>
                  </div>
        ...
        <div style={{ position: "fixed", right: 20, top: "50%", transform: "translateY(-50%)", zIndex: 1200, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button
            ...
            aria-label="Open quick jump for Privacy Policy sections"
            title="Quick Jump"
          >
            <AppstoreOutlined />
          </button>
