...
export default function IndianGolfAddEditData() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        setIsFetching(true);
        const res = await listIndianGolf({ tour_type: tourType });
  ...
  const copyFromMainTour = async () => {
    try {
      setIsFetching(true);
      const res = await listIndianGolf({ tour_type: "M" });
      if (!res?.status || !res?.result?.id) {
        notification.warning({ message: "Main Tour data not found", description: "Please save the Main Tour Indian Golf page first.", placement: "topRight", duration: 3 });
        return;
      }
      hydrateForm(res.result);
      setId("");
