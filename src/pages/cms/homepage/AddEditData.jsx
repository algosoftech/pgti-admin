...
const CDN_BASE = (() => {
  const base = process.env.REACT_APP_BUNNY_CDN_PULL_ZONE || "";
  const normalized = base && !/^https?:\/\//i.test(base) ? `https://${base}` : base;
...
const resolveMediaSrc = (value = "") => {
  ...
  if (CDN_BASE && !value.startsWith("/")) return `${CDN_BASE}/${value.replace(/^\//, "")}`;
              ...
              {heroVideo.url ? (
                <div className="row">
                  <div className="col-lg-5 col-12 mb-3">
                    ...
                    >
                      <video
                        src={resolveMediaSrc(heroVideo.url)}
                        controls
