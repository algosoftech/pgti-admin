...
import { addEditTermsConditions, listTermsConditions } from "services/termsConditions.service";
...
export default function TermsConditionsAddEditData() {
  ...
  useEffect(() => {
    document.title = `PGTI || ${id ? "Edit" : "Setup"} Terms & Conditions`;
  }, [id]);
  ...
  useEffect(() => {
    ...
    const loadRecord = async () => {
      ...
      try {
        setIsFetching(true);
        const res = await listTermsConditions({ tour_type: tourType });
  ...
  const copyFromMainTour = async () => {
    try {
      setIsFetching(true);
      const res = await listTermsConditions({ tour_type: "M" });
      if (!res?.status || !res?.result?.id) {
        notification.warning({
          message: "Main Tour data not found",
          description: "Please save the Main Tour Terms & Conditions page first.",
          placement: "topRight",
          duration: 3,
        });
        return;
  ...
  const saveSection = (sectionKey) => {
    Modal.confirm({
      ...
      content: "Do you really want to edit and save these changes for this Terms & Conditions section?",
      ...
      onOk: async () => {
        ...
        try {
          ...
          const res = await addEditTermsConditions({
          ...
          if (res?.status === true) {
            ...
            notification.open({
              message: "Success",
              description: "Terms & Conditions section saved successfully.",
              placement: "topRight",
            ...
            notification.open({
              message: "Oops!",
              description: res?.message || "Failed to save Terms & Conditions.",
              placement: "topRight",
              icon: <InfoCircleOutlined style={{ color: "red" }} />,
              duration: 2,
            });
    ...
    <div className="admin-page-container">
      <div className="page-header">
        <div className="d-flex justify-content-between align-items-center">
          <div>
            ...
            <p className="page-subtitle">Manage the Terms & Conditions page content section by section.</p>
            {isFetching && <p className="page-subtitle" style={{ marginTop: 6 }}>Loading saved Terms & Conditions data...</p>}
          </div>
      ...
      >
        ...
        <button
          ...
          aria-label="Open quick jump for Terms & Conditions sections"
          title="Quick Jump"
        >
          <AppstoreOutlined />
        </button>
