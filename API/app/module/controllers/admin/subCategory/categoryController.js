...
exports.addEditData = async function (req, res) {
    try {
        const userId = req.user.userId;
        const {editId, category, name, seq_order, image} = req.body;
        if(!name){
            ...
            if(!editId) {
                ...
                const dataCount = await commonServices.select(where, 'sub_categories');
                if(dataCount > 0){
                    ...
                    const result = await commonServices.insert(insertParam, 'sub_categories');
                    return response.sendResponse(res, response.build("SUCCESS", { result }));
                ...
                if(dataCount > 0){
                    ...
                    const result = await commonServices.update(updateParam, 'sub_categories');
                    return response.sendResponse(res, response.build("SUCCESS", { result }));
                } else{
                    return response.sendResponse(res, response.build("ERROR_ALREADY_EXIST", {  }));
                }
