...
exports.addEditData = async function (req, res) {
    try {
        const userId = req.user.userId;
        const {editId, image, type, page} = req.body;
        if(!image){
            ...
            if(!editId) {
                ...
                const result = await commonServices.insert(insertParam, 'banners');
                return response.sendResponse(res, response.build("SUCCESS", { result }));
            } else {
                const where = {
                    type : "count",
                ...
                const dataCount = await commonServices.select(where, 'banners');
                if(dataCount > 0){
                    const updateParam = {
                    ...
                    const result = await commonServices.update(updateParam, 'banners');
                    return response.sendResponse(res, response.build("SUCCESS", { result }));
                } else{
