...
exports.addEditData = async function (req, res) {
    try {
        const userId = req.user.userId;
        const {editId, question, answer, tag} = req.body;
        if(!question){
            return response.sendResponse(res, response.build("QUESTION_EMPTY", { }));
        } else if(!answer){
            return response.sendResponse(res, response.build("ANSWER_EMPTY", { }));
        } else{
            ...
            if(!editId) {
                ...
                const result = await commonServices.insert(insertParam, 'faqs');
                return response.sendResponse(res, response.build("SUCCESS", { result }));
            } else {
                const where = {
                    type : "count",
                ...
                const dataCount = await commonServices.select(where, 'faqs');
                if(dataCount > 0){
                    const updateParam = {
                    ...
                    const result = await commonServices.update(updateParam, 'faqs');
                    return response.sendResponse(res, response.build("SUCCESS", { result }));
                } else{
...
exports.delete = async function (req, res) {
    try {
        ...
        if(!userId){
            ...
            const dataCount = await commonServices.select(where, 'faqs');
            if(dataCount > 0){
                const result = await commonServices.delete({ id : parseInt(editId) }, 'faqs');
                return response.sendResponse(res, response.build("SUCCESS", { result : { message: "FAQ deleted successfully" } }));
            } else{
                return response.sendResponse(res, response.build("ERROR_DATA_NOT_FOUND", {  }));
            }
        }
