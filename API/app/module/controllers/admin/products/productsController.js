...
exports.list = async function (req, res) {
    try {
        ...
        if(!userId){
            ...
            const result = await commonServices.select(where, 'products');
            if(result && result?.length > 0){
                ...
                const count = await commonServices.select(option, 'products');
...
exports.addEditData = async function (req, res) {
    try {
        const userId = req.user.userId;
        const {editId, image, title, description, category, subCategory, made_two_order, subscription_products, fresh_item } = req.body;
        if(!title){
            ...
            const params = {
                ...
                made_two_order          : made_two_order || "N", 
                subscription_products   : subscription_products || "N", 
                fresh_item              : fresh_item || "N",
            ...
            if(!editId) {
                ...
                const dataCount = await commonServices.select(where, 'products');
                if(dataCount > 0){
                    ...
                    const result = await commonServices.insert(insertParam, 'products');
                    return response.sendResponse(res, response.build("SUCCESS", { result }));
                }
            } else {
                const where = {
                ...
                if(dataCount > 0){
                    ...
                    const result = await commonServices.update(updateParam, 'products');
                    return response.sendResponse(res, response.build("SUCCESS", { result }));
                } else{
                    return response.sendResponse(res, response.build("ERROR_ALREADY_EXIST", {  }));
                }
